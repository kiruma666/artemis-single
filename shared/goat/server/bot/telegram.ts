...
import {findPendingTelegramUsers, findUserByTelegramUserId, findUserByWalletAddress, isTelegramGroupJoined} from 'server/mongo/user/model';
import {
    STAGE,
    TELE_TOKEN,
    TELE_GROUP_ID,
    TELE_GROUP_INVITE_LINK,
    TELE_USER_PENDING_SECONDS,
    WEBSITE_ORIGIN,
    TELE_ID_ALERT
...
const getWelcomeMessage = () =>
    `Welcome to the GOAT Club campaign! First, join our official group, then return here to check your verification status. Please click the link below to join the official group.
${TELE_GROUP_INVITE_LINK}`;
...
const getVerifiedMismatchedMessage = (walletAddress: string) => `Welcome to the GOAT Club campaign! Your account has been verified by this wallet address: ${getMaskedAddress(walletAddress)}`;
...
async function findGroupMember(teleUserId?: any) {
    console.log(`[Telegram][getChatMember] teleUserId=${teleUserId}, groupId=${TELE_GROUP_ID}`);
    ...
    const member = await bot.api.getChatMember(TELE_GROUP_ID, Number(teleUserId));
    const isMember = isTelegramGroupJoined(member);
    console.log(`[Telegram][getChatMember] teleUserId=${teleUserId}, groupId=${TELE_GROUP_ID}, isMember=${isMember}`, member);
...
export default function startTelegramBot() {
    if (!TELE_TOKEN || !TELE_GROUP_ID || !TELE_GROUP_INVITE_LINK) {
    ...
    bot = new Bot<Context>(TELE_TOKEN);
    ...
    bot.on('chat_member', catchAsync(async ctx => {
        ...
        if (!!TELE_GROUP_ID && ctx.chat.id === +TELE_GROUP_ID) {
            const teleUserId = ctx.chatMember.new_chat_member.user.id;
            const targetTeleUser = await findUserByTelegramUserId(`${teleUserId}`);
            const pendingUsers = await findPendingTelegramUsers(new Date(Date.now() - +TELE_USER_PENDING_SECONDS * 1e3));
...
export function sendAlertMessage(message: string) {
    console.log(message);

    if (!TELE_ID_ALERT || !bot) {
        return;
    }

    return bot.api.sendMessage(TELE_ID_ALERT, message);
}
