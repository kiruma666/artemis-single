...
import {BIU_WEB_ORIGIN} from 'server/util/constant';
...
export const USER_MODEL_NAME = 'biu_user';
...
export const pickAndDeriveUserFields = (user: User, inviteCode: string) => {
    ...
    return {
        ...
        inviteCode,
        invitedBy,
        inviteLink: BIU_WEB_ORIGIN && `${BIU_WEB_ORIGIN}?inviteCode=${inviteCode}`,
...
export const findUserByWalletAddressIgnoreCase = async (walletAddress?: string) => {
    console.log(`[User] findUserByWalletAddress: walletAddress=${walletAddress}`);
    const user = await UserModel.findOne({walletAddress: {$regex: new RegExp(`^${walletAddress}$`, 'i')}});
