 ...
 * @Last Modified time: 2024-10-12 09:44:33
 */
...
async function fetchAndSaveHolderEPendleBalance() {
    ...
    try {
            ...
            for (const [addressMap, chainHolderMap, key] of addressMapChainHolderEntries) {
                await Promise.all(Object.entries(addressMap).map(async ([chain, address]) => {
                    try {
                        ...
                        sendErrorMessage(`fetchHolderEPendleBalance ${chain} ${key} error`, error);
                    }
                }));
            }
        ...
        console.log('fetchHolderEPendleBalance done');
        ...
        try {
            ...
            console.log('saveHolderEPendleBalance done');
        } catch (error) {
            sendErrorMessage('saveHolderEPendleBalance error', error);
        }
    } catch (error) {
        sendErrorMessage('fetchAndSaveHolderEPendleBalance error', error);
    } finally {
        fetching = false;
    }
}

export function mountEuclidHolderEPendleBalance(app: Application) {
    if (process.env.STAGE === 'prod') {
        ...
        console.log('fetchAndSaveHolderEPendleBalance started');
    }

    const apiPathPrefix = '/api/euclid-holder-e-pendle-balance';
    app.get(`${apiPathPrefix}/trigger`, async (req, res) => {
        ...
        res.setHeader('Content-Disposition', `attachment; filename=euclid-holder-e-pendle-balance-${dateStart}-${endDate.toISOString().split('T')[0]}.csv`);
