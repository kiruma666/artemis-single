 ...
 * @Last Modified time: 2024-02-28 22:57:49
 */
...
async function fetchAndSaveHolderVlEqbBalance() {
    ...
    try {
        ...
        while (start < holders.length) {
            ...
            console.log('fetchAndSaveHolderVlEqbBalance', start, end, batchHolders.length);
            await Promise.all(Object.entries(vlEqbAddressMap).map(async ([chain, address]) => {
                try {
                    ...
                    sendErrorMessage(`fetchHolderVlEqbBalance ${chain} error`, error);
                }
            }));
        ...
        console.log('fetchHolderVlEqbBalance done');
        ...
        try {
            ...
            console.log('saveHolderVlEqbBalance done');
        } catch (error) {
            sendErrorMessage('saveHolderVlEqbBalance error', error);
        }
    } catch (error) {
        sendErrorMessage('fetchAndSaveHolderVlEqbBalance error', error);
    } finally {
        fetching = false;
    }
}

export function mountEuclidHolderVlEqbBalance(app: Application) {
    if (process.env.STAGE === 'prod') {
        ...
        console.log('fetchAndSaveHolderVlEqbBalance started');
    }

    const apiPathPrefix = '/api/euclid-holder-vl-eqb-balance';
    app.get(`${apiPathPrefix}/trigger`, async (req, res) => {
        ...
        res.setHeader('Content-Disposition', `attachment; filename=euclid_holder_vl_eqb_balance_${dateStart}_${endDate.toISOString().split('T')[0]}.csv`);
