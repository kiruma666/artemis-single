 ...
 * @Last Modified time: 2024-04-04 15:00:09
 */
...
import {ArtemisAssetDepositorScript, getUserInviterMap} from '..';
...
export async function calculateGroupRanking() {
    ...
    const groups: any[] = groupIdList.map(groupId => {
        const totalMetis = depositorRecords.filter(record => record.user === groupId).reduce((acc, record) => {
            return acc.add(BigNumber.from(record.amount));
        }, BigNumber.from(0)).toString();

        return {
            group: groupId,
            totalMetis: totalMetis.toString()
        };
    });
    groups.sort((a, b) => BigNumber.from(b.totalMetis).gte(BigNumber.from(a.totalMetis)) ? 1 : -1);
    groups.forEach((group, index) => {
        group.rank = index + 1;
        group.currentBoost = CurrentBoostArr[index] || 0;
    });
    console.log('Artemis calculateGroupRanking calculate done');

    await groupRankingModel.create({groups});
    console.log('Artemis calculateGroupRanking save done');
}

export function mountArtemisHolderGroupRanking(app: Application) {
    if (process.env.STAGE === 'prod') {
        ...
        console.log('Artemis calculateGroupRanking started');
    }

    const apiPathPrefix = '/api/artemis-holder-group-ranking';
    app.get(`${apiPathPrefix}/trigger`, async (req, res) => {
        ...
        const headers = ['rank', 'group', 'totalMetis', 'currentBoost', 'createdAt'] as const;
        ...
        res.setHeader('Content-Disposition', `attachment; filename=artemis-holder-group-ranking-${groupRanking?.createdAt.toISOString().split('T')[0]}.csv`);
